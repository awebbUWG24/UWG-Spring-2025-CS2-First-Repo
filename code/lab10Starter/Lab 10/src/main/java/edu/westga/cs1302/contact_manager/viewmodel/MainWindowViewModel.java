...
public class MainWindowViewModel {
	...
	/** Return the hash map containing all contacts added to the system
	 * 
	 * @precondition none
	 * @postcondition none
	 * 
	...
	public String findContact() {
		...
		if (this.contactMap.containsKey(this.searchCriteria.get())) {
			...
			for (Entry<String, Contact> currContact : this.contactMap.entrySet()) {
				if (currContact.getValue().getName().equals(this.searchCriteria.get())) {
					foundContact = currContact.getValue().toString();
				}
			}
		}
		return foundContact;
